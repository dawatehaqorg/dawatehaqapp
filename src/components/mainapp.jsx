...
const blogPosts = [
  ...
  const maxSlide = Math.max(0, blogPosts.length - getVisible());
...
`}</style>
      ...
      <div id="home" className="hero">
        ...
        <div className="hero-content">
          ...
          <h1 className="hero-title">
            Serving Islam,<br/><span>Serving Humanity</span>
          </h1>
          ...
          <div className="hero-actions">
            <button className="btn-primary" onClick={() => scrollTo("programs")}>Explore Our Work</button>
            <button className="btn-outline" onClick={() => scrollTo("donate")}>Donate Now</button>
          </div>
        </div>
     ...
     {/* AIMS & OBJECTIVES */}
      <section id="objectives" style={{background:"var(--cream)",padding:"100px 5%"}}>
        ...
        <div className="blog-inner">
          ...
          <div className="blog-slider-wrap" ref={blogRef}>
            <div className="blog-track-outer"
              onTouchStart={e=>setBlogTouch({x:e.touches[0].clientX,dragging:true})}
              onTouchEnd={e=>{
                if(!blogTouch.dragging) return;
                const diff = blogTouch.x - e.changedTouches[0].clientX;
                if(diff > 3) nextBlog();
                else if(diff < 5) prevBlog();
                setBlogTouch({x:0,dragging:false});
              }}>
              <div
                ...
                {blogPosts.map((post, i) => (
                  <div key={i} className="blog-card">
                    ...
                    {/* Pattern overlay */}
                    <div style={{position:"absolute",inset:0,backgroundImage:`url('${encodedPattern}')`,backgroundSize:"40px 40px",opacity:0.2}}/>
